...
void window_main_button_buy_clicked_cb() {
  ...
  int space =
      window_main.game_state->pocket_capacity - window_main.game_state->pocket;
  ...
  QObject::connect(
      window_input.button_ok, &QPushButton::clicked,
      [drug_idx, price, max_buy, j, d]() {
        ...
        if (count > 0 && count <= max_buy && window_main.game_state) {
          ...
          window_main.game_state->player_price[drug_idx] =
          ...
          if (window_main.game_state->drug_table[drug_idx][j][d].qty <= 0) {
            window_main.game_state->drug_table[drug_idx][j][d].available =
                false;
          }
...
void window_main_button_dump_clicked_cb() {
  ...
  QObject::connect(
      window_input.button_ok, &QPushButton::clicked, [drug_idx, owned_qty]() {
        int count = window_input.spinbutton_value->value();
        if (count > 0 && count <= owned_qty && window_main.game_state) {
          window_main.game_state->pocket -= count;
          window_main.game_state->player_qty[drug_idx] -= count;
...
void menuitem_places_finances_activate_cb() {
  ...
  auto update_finance_labels = []() {
    if (!window_main.game_state) return;
    window_finance.label_cash->setText(
        QString::fromStdString(money_string(window_main.game_state->cash)));
    window_finance.label_bank->setText(
        QString::fromStdString(money_string(window_main.game_state->bank)));
    window_finance.label_debt->setText(
        QString::fromStdString(money_string(window_main.game_state->debt)));
  };

  update_finance_labels();

  QObject::connect(
      window_finance.button_doit, &QPushButton::clicked,
      [update_finance_labels]() {
        if (!window_main.game_state) return;
  ...
  QObject::connect(window_finance.button_repay, &QPushButton::clicked,
                   [update_finance_labels]() {
                     if (!window_main.game_state) return;
                     int amount = std::min(window_main.game_state->cash,
                                           window_main.game_state->debt);
                     window_main.game_state->cash -= amount;
  ...
  QObject::connect(window_finance.button_borrow, &QPushButton::clicked,
                   [update_finance_labels]() {
                     if (!window_main.game_state) return;
                     int amount = 2000;
                     window_main.game_state->cash += amount;
...
void menuitem_places_shopping_activate_cb() {
  ...
  window_shopping.label_cash->setText(
      QString::fromStdString(money_string(window_main.game_state->cash)));
...
void menuitem_places_hospital_activate_cb() {
  ...
  window_hospital.label_cash->setText(
      QString::fromStdString(money_string(window_main.game_state->cash)));
  ...
  QObject::connect(
      window_hospital.scalebutton_health, &QSlider::valueChanged, [](int val) {
        if (!window_main.game_state) return;
        int needed = std::max(0, val - window_main.game_state->health);
        int cost = needed * 50;
        window_hospital.progressbar_health->setValue(val);
        window_hospital.label_cost->setText(
            QString::fromStdString(money_string(cost)));
      });
...
void menuitem_places_vault_activate_cb() {
    ...
    for (int i = 0; i < DRUG_NUM; ++i) {
      if (window_main.game_state->player_qty[i] > 0) {
        QTreeWidgetItem* item =
            new QTreeWidgetItem(window_vault.treeview_pocket);
        item->setText(0, QString::fromStdString(drug_name(drug_info[i].id)));
        item->setText(1,
                      QString::number(window_main.game_state->player_qty[i]));
        item->setText(2, QString::fromStdString(money_string(
                             window_main.game_state->player_price[i])));
        item->setData(0, Qt::UserRole, i);
      }
      if (window_main.game_state->vault_qty[i] > 0) {
        QTreeWidgetItem* item =
            new QTreeWidgetItem(window_vault.treeview_vault);
        item->setText(0, QString::fromStdString(drug_name(drug_info[i].id)));
  ...
  QObject::connect(window_vault.button_intovault, &QPushButton::clicked,
                   [update_vault_lists]() {
                     if (!window_main.game_state) return;
                     ...
                     if (drug_idx >= 0 && drug_idx < DRUG_NUM &&
                         window_main.game_state->player_qty[drug_idx] > 0) {
  ...
  QObject::connect(window_vault.button_fromvault, &QPushButton::clicked,
                   [update_vault_lists]() {
                     if (!window_main.game_state) return;
                     ...
                     int drug_idx = item->data(0, Qt::UserRole).toInt();
                     if (drug_idx >= 0 && drug_idx < DRUG_NUM &&
                         window_main.game_state->vault_qty[drug_idx] > 0) {
                       if (window_main.game_state->pocket <
  ...
  QObject::connect(window_vault.button_ok, &QPushButton::clicked,
                   window_vault.window, &QDialog::close);
  ...
  for (int i = 0; i < DRUG_NUM; ++i) {
    if (window_main.game_state->vault_qty[i] > 0) {
      ...
      info += std::format("  {}: {}\n", drug_name(drug_info[i].id),
                          window_main.game_state->vault_qty[i]);
    }
  }
  if (empty) info += "  (Vault is empty)";
  QMessageBox::information(window_main.window, "Vaults",
...
void menuitem_info_world_drug_prices_activate_cb() {
  ...
  QObject::connect(window_world_drug_prices.treeview_drug,
                   &QTreeWidget::itemClicked, [&](QTreeWidgetItem* item) {
                     if (!window_main.game_state) return;
                     int drug_idx = item->data(0, Qt::UserRole).toInt();
                     fill_treeview_city_list(
                         window_world_drug_prices.treeview_city,
...
void menuitem_info_world_cities_activate_cb() {
  ...
  QObject::connect(window_world_cities.treeview_city, &QTreeWidget::itemClicked,
                   [](QTreeWidgetItem* item) {
                     if (!window_main.game_state) return;
...
void menuitem_info_shipment_status_activate_cb() {
  QMessageBox::information(window_main.window, "Shipment Status",
                           "No active shipments.");
}

void menuitem_info_history_activate_cb() {
  QMessageBox::information(window_main.window, "History",
...
void window_main_button_stayhere_clicked_cb() {
  ...
  if (window_main.game_state->day >= DAY_NUM - 1) {
    int score = window_main.game_state->cash + window_main.game_state->bank -
                window_main.game_state->debt;
    if (score < 0) score = 0;
    std::string msg = std::format(
        "That's it, the game is over! You have a final score of ${}",
        money_string(score));
    QMessageBox::information(window_main.window, "Game Over",
...
void window_main_button_about_clicked_cb() {
  std::string info =
      std::format("{}\n{}\n\nAuthor: {}\nEmail: {}\nVersion: {}", kProgramName,
                  ...
                  kProgramVersion);
  QMessageBox::about(window_main.window, "About Drux Lord",
                     QString::fromStdString(info));
}

void window_main_button_docs_clicked_cb() {
  QMessageBox::information(
      window_main.window, "Documentation",
      QString::fromUtf8(
